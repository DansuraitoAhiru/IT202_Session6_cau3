...
-- cụ thể là mỗi lần status = 'CANCELLED' thì sẽ trả về 1 hoặc 0 nếu dùng count nếu dùng ko chuẩn có thể sẽ bị đếm cả những lần trả về 0
...
sum(CASE WHEN status = 'CANCELLED' THEN 1 ELSE 0 END) as total_cancelled    -- dùng COUNT(CASE WHEN status = 'CANCELLED' THEN 1 END) cũng đc
FROM bookings
GROUP BY user_id
